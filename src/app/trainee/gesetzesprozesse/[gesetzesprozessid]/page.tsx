...
export default function TraineegeschäftsprozesseDetailPage() {
  ...
  const [item, setItem] = useState<{ id: string; title: string; descriptionText: string; isActive?: boolean; durationValue?: number | null; durationUnit?: 'DAYS' | 'WEEKS' | null; activatedAt?: string | null } | null>(null);
  ...
  if (!item) return <div className="p-6">Nicht gefunden</div>;
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 p-6">
      <div className="rounded-3xl border border-accent/30 bg-black/30 p-5">
        ...
        <p className="text-muted-foreground mt-2 whitespace-pre-line">{item.descriptionText}</p>
        {item.isActive && item.durationValue && item.activatedAt && (
          <div className="mt-3 text-sm">
            {(() => {
              const started = new Date(item.activatedAt as string).getTime();
              const now = Date.now();
              const daysElapsed = Math.floor((now - started) / (1000 * 60 * 60 * 24));
              const total = Number(item.durationValue || 0);
