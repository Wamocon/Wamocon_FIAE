...
export default function TraineeUseCaseDetailPage() {
  ...
  const ucUrl =
    profile?.id && useCaseId
      ? `/api/trainee/use-cases/${useCaseId}?traineeId=${profile.id}`
      : null;
  const docsUrl =
    profile?.id && useCaseId
      ? `/api/trainee/use-cases/${useCaseId}/documents?traineeId=${profile.id}`
      : null;
  ...
  const {
    ...
    isLoading: loading,
    error: queryError,
  } = useApiQuery<UseCaseResponse>(ucUrl);
  ...
  useEffect(() => {
    ...
    if (sub) {
      ...
      const mapped = (sub.links || []).map((l: any) => ({
        url: l.url as string,
  ...
  const submit = async () => {
    ...
    try {
      ...
      const r = await fetch(`/api/trainee/use-cases/${useCaseId}/submit`, {
        method: 'POST',
  ...
  if (queryError)
    return <div className="p-6 text-red-500">{queryError.message}</div>;
  if (!useCase) return <div className="p-6">{t('common.notFound')}</div>;

  return (
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <div className="border-accent/30 bg-card rounded-3xl border p-5">
              ...
              <button
                ...
                onClick={() =>
                  setSelectedPdfUrl(prev =>
                    prev === doc.storageUrl ? null : doc.storageUrl
                  )
                }
                className={`inline-flex max-w-full items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium transition-all duration-200 ${selectedPdfUrl === doc.storageUrl ? 'bg-accent/15 text-accent border-accent/40 border' : 'bg-primary text-primary-foreground shadow-primary/20 shadow-md hover:scale-[1.02] hover:shadow-lg'}`}
              >
                <ChevronLeft
                  className={`h-4 w-4 shrink-0 transition-transform duration-200 ${selectedPdfUrl === doc.storageUrl ? 'rotate-[-90deg]' : 'rotate-90'}`}
                />
        ...
        {/* Inline PDF Viewer */}
        {selectedPdfUrl && (
          <div className="animate-in fade-in slide-in-from-top-4 mt-4 duration-300">
            <div className="border-accent/30 bg-card relative h-[80vh] w-full overflow-hidden rounded-xl border shadow-2xl">
              <iframe
        ...
        {useCase.isActive && useCase.durationValue && useCase.activatedAt && (
          <div className="border-accent/10 mt-4 border-t pt-4 text-sm">
            {(() => {
              ...
              return (
                <span className="text-muted-foreground">
                  {t('useCase.timeRemaining')}{' '}
                  <span
                    className={
                      left < 3 ? 'font-bold text-red-400' : 'text-foreground'
                    }
                  >
                    {left} {t('useCase.days')}
                  </span>{' '}
                  • {t('useCase.dueOn')} {dueDate.toLocaleDateString()}
                </span>
              );
            })()}
          </div>
      ...
      {success && (
        <div className="rounded-md border border-green-500/40 bg-green-500/10 p-3 text-green-300">
          {success}
        </div>
      )}
      {error && (
      ...
      <div className="border-accent/30 bg-card space-y-4 rounded-3xl border p-5">
        <div>
          <label className="mb-1 block text-sm font-medium">
            {t('useCase.submission')}
          </label>
          <textarea
            value={submissionText}
            onChange={e => setSubmissionText(e.target.value)}
            className={`border-accent/30 bg-muted text-foreground w-full rounded-xl border px-3 py-2 ${!canEdit ? 'cursor-not-allowed opacity-60' : ''}`}
            rows={6}
            disabled={!canEdit}
          />
        </div>
        <div className="space-y-2">
          ...
          {links.map((l, i) => (
            <div key={i} className="grid grid-cols-1 gap-2 md:grid-cols-3">
              <input
                className={`border-accent/30 bg-muted text-foreground rounded-xl border px-3 py-2 md:col-span-2 ${!canEdit ? 'cursor-not-allowed opacity-60' : ''}`}
              ...
              />
              <input
                className={`border-accent/30 bg-muted text-foreground rounded-xl border px-3 py-2 ${!canEdit ? 'cursor-not-allowed opacity-60' : ''}`}
          ...
          {canEdit && (
            <div className="flex gap-2">
              <button
                className="border-accent/30 hover:bg-background/60 rounded-md border px-3 py-2 text-sm"
                onClick={() =>
      ...
      {useCase.courseId && (
        <div className="mt-8 flex justify-center pb-8">
          <Link
            href={`/trainee/modules/${useCase.courseId}`}
            className="text-muted-foreground hover:text-accent flex items-center gap-2 text-sm font-medium transition-colors"
          >
