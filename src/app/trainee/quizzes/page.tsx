...
export default function TraineeQuizzesPage() {
  ...
  const quizzes = mockData.curriculum.flatMap(module => 
    module.chapters?.flatMap(chapter => 
      chapter.lessons?.filter(lesson => lesson.type === 'quiz').map(lesson => ({
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-8">
      ...
      <div className="glass-effect rounded-3xl p-8 shadow-lg border border-accent/30">
        <div className="flex items-center gap-6 mb-6">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground mb-2">Quizze & Tests</h1>
            <p className="text-muted">Testen Sie Ihr Wissen mit verschiedenen Quizzen und bewerten Sie Ihren Fortschritt</p>
          </div>
        </div>
      </div>
          ...
          <div key={quiz.id} className="glass-effect rounded-3xl p-6 shadow-lg border border-accent/30 hover:shadow-xl transition-all duration-300">
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-bold text-xl text-foreground">{quiz.title}</h3>
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between text-sm mb-2">
                ...
                <span className="font-medium text-foreground">{quiz.bestScore || 0}%</span>
              </div>
            ...
            <div className="grid grid-cols-3 gap-4 mb-4 text-sm">
              <div className="text-center p-3 bg-background/50 rounded-xl">
                <div className="text-lg font-bold text-accent">{quiz.questions?.length || 0}</div>
                <div className="text-muted">Fragen</div>
              </div>
              <div className="text-center p-3 bg-background/50 rounded-xl">
                <div className="text-lg font-bold text-primary">{quiz.timeLimit}</div>
                <div className="text-muted">Zeit</div>
              </div>
              <div className="text-center p-3 bg-background/50 rounded-xl">
                <div className="text-lg font-bold text-accent">{quiz.attempts || 0}</div>
