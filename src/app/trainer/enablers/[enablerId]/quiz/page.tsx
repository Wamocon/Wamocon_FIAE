...
export default function EnablerQuizBuilderPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const res = await fetch(`/api/trainer/enablers/${enablerId}/quiz`, { cache: 'no-store' });
        ...
        if (data.quiz) {
          ...
          setQuestions(
            (data.quiz.questions || []).map((q: any) => ({
              ...
              options: (q.options || []).map((o: any) => o.optionText) as [string, string, string, string],
              correctIndex: Math.max(0, (q.options || []).findIndex((o: any) => o.isCorrect)),
            }))
          );
        }
      } catch (e: any) {
  ...
  const addQuestion = () => setQuestions((prev) => [...prev, { questionText: '', options: ['', '', '', ''], correctIndex: 0 }]);
  const removeQuestion = (idx: number) => setQuestions((prev) => prev.filter((_, i) => i !== idx));
  const updateQuestion = (idx: number, patch: Partial<BuilderQuestion>) =>
    setQuestions((prev) => prev.map((q, i) => (i === idx ? ({ ...q, ...patch }) : q)));
  const updateOption = (qi: number, oi: number, value: string) =>
    setQuestions((prev) => prev.map((q, i) => i === qi ? ({ ...q, options: q.options.map((o, j) => j === oi ? value : o) as any }) : q));
  ...
  const save = async () => {
    ...
    const cleaned = questions
      ...
      .filter((q) => q.questionText.length > 0 && q.options.every((o) => o.length > 0));
    if (cleaned.length === 0) return setError('Bitte mindestens eine vollständige Frage hinzufügen.');
    ...
    try {
      ...
      const res = await fetch(`/api/trainer/enablers/${enablerId}/quiz`, {
        ...
        body: JSON.stringify({ title: title.trim(), createdById: profile.id, questions: cleaned }),
      });
  ...
  return (
    <div className="mx-auto max-w-3xl p-6 space-y-6">
      ...
      <div>
        ...
        <input value={title} onChange={(e) => setTitle(e.target.value)} className="w-full rounded-md border border-border bg-background px-3 py-2" />
      </div>
      <div className="space-y-6">
        {questions.map((q, i) => (
          <div key={i} className="rounded-xl border border-border p-4">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => removeQuestion(i)} className="text-sm rounded-md border border-border px-2 py-1">Entfernen</button>
            </div>
            <input
        ...
        <button type="button" onClick={addQuestion} className="rounded-md border border-border px-3 py-2 text-sm">+ Frage hinzufügen</button>
      </div>
