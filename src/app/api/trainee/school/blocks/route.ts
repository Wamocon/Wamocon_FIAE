...
// Helper to calculate the module phase for a block date.
function calculateAusbildungsjahr(
  startDate: Date,
  durationYears?: number | null,
  atDate: Date = new Date()
): number {
  return getTrainingPhase(startDate, durationYears, atDate);
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (ausbildungsjahr) {
      conditions.push(
        eq(ausbildungBlocks.ausbildungsjahr, parseInt(ausbildungsjahr, 10))
      );
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!traineeId)
      return NextResponse.json(
        { error: 'traineeId required' },
        { status: 400 }
      );
    if (!calendarWeek || calendarWeek < 1 || calendarWeek > 53) {
      return NextResponse.json(
        { error: 'Valid calendarWeek (1-53) required' },
        { status: 400 }
      );
    }
    if (!year)
    ...
    if (!blockType || !validBlockTypes.includes(blockType)) {
      return NextResponse.json(
        { error: `Valid blockType required (${validBlockTypes.join(', ')})` },
        { status: 400 }
      );
    }
    ...
    if (blockType === 'SONSTIGES' && !description) {
      return NextResponse.json(
        { error: 'Description required for SONSTIGES block type' },
        { status: 400 }
      );
    }
    ...
    // Get trainee profile to calculate the phase from the block date.
