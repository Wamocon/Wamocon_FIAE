...
function determineBlockType(
  ...
  notes: string
): 'SCHOOL' | 'COMPANY' | 'HOLIDAY' | 'EXAM' | 'PERSONAL' {
  ...
  try {
    ...
    for (let i = 0; i < csvData.length; i++) {
      ...
      try {
        ...
        // Skip empty rows before calculating the duration-aware phase column.
        if (!kw || isNaN(kw)) {
          result.skipped++;
          continue;
        }
        ...
        if (!dateRange) {
          result.errors.push({
            row: rowNum,
            error: `Invalid date format: ${datumStr}`,
          });
          continue;
        }
        ...
        const yearColumn =
          ausbildungsjahr === 1 ? mapping.year1 : mapping.year2;
