...
import { Geschäftsprozesse, geschäftsprozesseSubmissions, geschäftsprozesseSubmissionLinks, courseMembers, notifications, courses } from '@/db/migrations/schemas/schema';

// POST submit a geschäftsprozesse submission (creates new attempt or updates with incremented attemptNumber)
// Body: { traineeId: string, submissionText?: string, links?: Array<{ url: string, description?: string }> }
export async function POST(req: NextRequest, { params }: { params: { gesetzesprozessid: string } }) {
  try {
  ...
  .where(and(eq(geschäftsprozesseSubmissions.geschäftsprozesseId, gesetzesprozessid as any), eq(geschäftsprozesseSubmissions.traineeId, traineeId as any)));
      ...
      const [created] = await tx
        .insert(geschäftsprozesseSubmissions)
  .values({ traineeId: traineeId as any, geschäftsprozesseId: gesetzesprozessid as any, submissionText: submissionText as any, status: 'PENDING' as any, attemptNumber: nextAttempt })
        .returning();
      ...
      if (links.length) {
        ...
        if (values.length) await tx.insert(geschäftsprozesseSubmissionLinks).values(values);
      }
      return submissionId;
    });
    ...
    try {
      ...
      if (trainerIds.size) {
        const values = Array.from(trainerIds).map((uid) => ({
          userId: uid,
          actorId: traineeId,
          type: 'geschäftsprozesse_SUBMITTED',
          title: 'geschäftsprozesse eingereicht',
          message: `Ein Trainee hat einen geschäftsprozesse eingereicht: ${g.title}`,
          linkUrl: '/trainer/reviews?view=Geschäftsprozesse&onlyPending=true',
          context: { geschäftsprozesseId: gesetzesprozessid, submissionId: result },
        }));
        await db.insert(notifications).values(values);
      }
    } catch (notifyErr) {
      console.warn('Failed to notify trainers for geschäftsprozesse submission', notifyErr);
    }

    return NextResponse.json({ ok: true, submissionId: result });
  } catch (e) {
    console.error('Trainee geschäftsprozesse submit POST error', e);
