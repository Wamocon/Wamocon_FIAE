...
export async function GET(
  ...
  try {
    ...
    const requesterId =
      searchParams.get('requesterId') || searchParams.get('trainerId');
    ...
    const progressPct =
      totalEnablers > 0
        ? Math.round((completedEnablers / totalEnablers) * 100)
        : 0;
    ...
    const latestUseCaseSubmissions = useCaseIds.length
      ...
      : [];
    const latestUseCaseById = new Map<
      string,
      (typeof latestUseCaseSubmissions)[number]
    >();
    ...
    const enablerQuizSubmissions = enablerQuizIds.length
      ? await db
          ...
          .where(
            and(
              eq(quizSubmissions.traineeId, traineeId as any),
              inArray(quizSubmissions.quizId, enablerQuizIds as any)
            )
          )
          .orderBy(desc(quizSubmissions.submittedAt))
      : [];
    const latestQuizSubByQuizId = new Map<
      string,
      (typeof enablerQuizSubmissions)[number]
    >();
    ...
    const globalQuizSubs = assignedQuizIds.length
      ? await db
          ...
          .where(
            and(
              eq(quizSubmissions.traineeId, traineeId as any),
              inArray(quizSubmissions.quizId, assignedQuizIds as any)
            )
          )
          .orderBy(desc(quizSubmissions.submittedAt))
      : [];
    const latestGlobalQuizById = new Map<
      string,
      (typeof globalQuizSubs)[number]
    >();
    ...
    const enablerTitleMap = new Map(
      enablerRows.map(e => [String(e.id), e.title])
    );
    ...
    const courseTitleMap = new Map(
      courseRows.map(c => [String(c.id), c.title])
    );

    // Pending counts
    const pendingUseCases = Array.from(latestUseCaseById.values()).filter(
      s => s.status === 'PENDING'
    ).length;
    const pendingQuizReviews = Array.from(latestQuizSubByQuizId.values())
      .concat(Array.from(latestGlobalQuizById.values()))
      .filter(s => s && !s.isReviewed).length;
    ...
    return NextResponse.json({
      ...
      enablers: await (async () => {
        ...
        const oRows = enablerIds.length
          ? await db
              .select({
                enablerId: traineeEnablerOverrides.enablerId,
                isActive: traineeEnablerOverrides.isActive,
              })
              .from(traineeEnablerOverrides)
              .where(
                and(
                  eq(traineeEnablerOverrides.traineeId, traineeId as any),
                  inArray(traineeEnablerOverrides.enablerId, enablerIds as any)
                )
              )
          : [];
        ...
        oRows.forEach(r =>
          oMap.set(
            String(r.enablerId),
            r.isActive === null ? null : Boolean(r.isActive)
          )
        );
        return enablerRows.map(e => {
          const override = oMap.get(String(e.id));
          const effectiveActive =
            typeof override === 'boolean' ? override : Boolean(e.isActive);
          return {
            id: e.id,
            title: e.title,
            courseId: e.courseId,
      ...
      useCases: await (async () => {
        const oRows = useCaseIds.length
          ? await db
              .select({
                useCaseId: traineeUseCaseOverrides.useCaseId,
                isActive: traineeUseCaseOverrides.isActive,
              })
              .from(traineeUseCaseOverrides)
              .where(
                and(
                  eq(traineeUseCaseOverrides.traineeId, traineeId as any),
                  inArray(traineeUseCaseOverrides.useCaseId, useCaseIds as any)
                )
              )
          : [];
        const oMap = new Map<string, boolean | null>();
        ...
        return useCaseRows.map(u => {
          ...
          const effectiveActive =
            typeof override === 'boolean' ? override : Boolean(u.isActive);
          return {
            id: u.id,
            title: u.title,
            isActive: effectiveActive,
