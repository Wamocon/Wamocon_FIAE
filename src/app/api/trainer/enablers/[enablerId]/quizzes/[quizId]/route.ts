  ...
  try {
    ...
    await db.transaction(async (tx) => {
      if (typeof body.title === 'string') {
        await tx.update(quizzes).set({ title: body.title }).where(eq(quizzes.id, quizId));
      }
      if (typeof body.isActive === 'boolean') {
        await tx.update(quizzes).set({ isActive: body.isActive }).where(eq(quizzes.id, quizId));
      }
      if (typeof body.difficulty === 'string') {
        await tx.update(enablerQuizLinks).set({ difficulty: body.difficulty }).where(eq(enablerQuizLinks.quizId, quizId));
      }

      if (Array.isArray(body.questions)) {
        // Replace questions fully for simplicity
        const existingQs = await tx.select().from(questions).where(eq(questions.quizId, quizId));
