...
export async function GET(
  ...
  try {
    ...
    const softSkillRadarData = Array.from(softSkillMap.entries()).map(
      ([area, data]) => {
        const avgGrade =
        ...
        return {
          ...
          radarValue:
            radarValue !== null ? Math.round(radarValue * 100) / 100 : null,
    ...
    const overallAverage =
      gradedComponents.length > 0
        ? gradedComponents.reduce((sum, r) => sum + (r.grade || 0), 0) /
    ...
    return NextResponse.json(
      { error: 'Interner Serverfehler beim Erstellen des Kompetenzprofils' },
      { status: 500 }
    );
  }
}
