...
export async function GET(
  ...
  try {
    ...
    const totalGradedComponents = components.filter(
      c => c.averageGrade !== null
    );
    const overallAverage =
      totalGradedComponents.length > 0
        ? totalGradedComponents.reduce(
            (sum, c) => sum + (c.averageGrade || 0),
            0
          ) / totalGradedComponents.length
        : null;
    ...
    try {
      ...
      const directRatings = await db
        ...
        .innerJoin(
          weeklyEvaluations,
          eq(weeklySoftskillRatings.weeklyEvaluationId, weeklyEvaluations.id)
        )
        .innerJoin(
          mesSoftskillCriteria,
          eq(
    ...
    try {
      const linkedRatings = await db
        ...
        .innerJoin(
          activityReports,
          eq(weeklyEvaluations.activityReportId, activityReports.id)
        )
        .innerJoin(
          mesSoftskillCriteria,
          eq(
    ...
    const softSkillsByArea: Record<
      string,
      { ratings: number[]; criteria: Set<string> }
    > = {};
    ...
    for (const rating of softSkillRatings) {
      ...
      if (numRating !== null && rating.competencyArea) {
        ...
        softSkillsByArea[rating.competencyArea]?.criteria.add(
          rating.criterionId
        );
    ...
    const softSkillAverages = {
      fachkompetenz:
        softSkillsByArea.FACHKOMPETENZ.ratings.length > 0
          ? Math.round(
              (softSkillsByArea.FACHKOMPETENZ.ratings.reduce(
                (a, b) => a + b,
                0
              ) /
                softSkillsByArea.FACHKOMPETENZ.ratings.length) *
                100
            ) / 100
          : null,
      methodenkompetenz:
        softSkillsByArea.METHODENKOMPETENZ.ratings.length > 0
          ? Math.round(
              (softSkillsByArea.METHODENKOMPETENZ.ratings.reduce(
                (a, b) => a + b,
                0
              ) /
                softSkillsByArea.METHODENKOMPETENZ.ratings.length) *
                100
            ) / 100
          : null,
      personalkompetenz:
        softSkillsByArea.PERSONALKOMPETENZ.ratings.length > 0
          ? Math.round(
              (softSkillsByArea.PERSONALKOMPETENZ.ratings.reduce(
                (a, b) => a + b,
                0
              ) /
                softSkillsByArea.PERSONALKOMPETENZ.ratings.length) *
                100
            ) / 100
          : null,
    };
    ...
    const allSoftSkillRatings = Object.values(softSkillsByArea).flatMap(
      a => a.ratings
    );
    const overallSoftSkillAverage =
      allSoftSkillRatings.length > 0
        ? Math.round(
            (allSoftSkillRatings.reduce((a, b) => a + b, 0) /
              allSoftSkillRatings.length) *
              100
          ) / 100
        : null;
    ...
    const softSkillCriteria = Array.from(criteriaMap.values())
      .map(c => ({
        ...
        averageGrade:
          c.ratings.length > 0
            ? Math.round(
                (c.ratings.reduce((a, b) => a + b, 0) / c.ratings.length) * 100
              ) / 100
            : null,
    ...
    return NextResponse.json({
      ...
      components,
      overallAverage:
        overallAverage !== null ? Math.round(overallAverage * 100) / 100 : null,
      shorteningEligible,
      ...
      gradeLegend: {
        '1': {
          label: 'Sehr gut',
          range: '92-100%',
          description: 'entspricht den Anforderungen in besonderem Maße',
        },
        '2': {
          label: 'Gut',
          range: '81-91%',
          description: 'entspricht den Anforderungen voll',
        },
        '3': {
          label: 'Befriedigend',
          range: '67-80%',
          description: 'entspricht den Anforderungen im Allgemeinen',
        },
        '4': {
          label: 'Ausreichend',
          range: '50-66%',
          description:
            'weist Mängel auf, entspricht aber noch den Anforderungen',
        },
        '5': {
          label: 'Mangelhaft',
          range: '30-49%',
          description:
            'entspricht nicht den Anforderungen, Grundkenntnisse vorhanden',
        },
        '6': {
          label: 'Ungenügend',
          range: '0-29%',
          description:
            'entspricht nicht den Anforderungen, Grundkenntnisse nicht ausreichend',
        },
      },
    });
  } catch (error: unknown) {
    ...
    return NextResponse.json(
      { error: 'Interner Serverfehler beim Aggregieren der Leistungsdaten' },
      { status: 500 }
    );
  }
}
