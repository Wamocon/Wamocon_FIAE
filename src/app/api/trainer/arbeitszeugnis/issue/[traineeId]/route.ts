...
export async function POST(
    ...
    try {
        ...
        const componentMap = new Map<string, {
            ...
            title: string;
            orderIndex: number;
            useCases: Array<{ letter: string; description: string; grade: string | null; hours: number }>;
            gradeSum: number;
            gradedCount: number;
        }>();
        ...
        const overallAverage = gradedComponents.length > 0
            ? gradedComponents.reduce((sum, c) => sum + (c.averageGrade || 0), 0) / gradedComponents.length
            : null;
