...
export async function POST(req: NextRequest) {
  try {
    ...
    const targetTraineeIds =
      providedTraineeIds || (traineeId ? [traineeId] : []);
    if (targetTraineeIds.length === 0)
      return NextResponse.json(
        { error: 'At least one traineeId required' },
        { status: 400 }
      );
      ...
      try {
        ...
        if (blockType === 'EXAM' && examSubType) {
          ...
          if (existing.length > 0) {
            errors.push(
              `Duplicate exam block for trainee ${tid}: ${examSubType} already exists.`
            );
            continue;
          }
        }
        ...
        try {
          ...
          console.warn(
            `Failed to notify trainee ${tid} for block creation`,
            notifyErr
          );
        }
    ...
    if (targetTraineeIds.length === 1 && results.length === 1) {
      return NextResponse.json(
        { id: results[0].id, block: results[0] },
        { status: 201 }
      );
    }
