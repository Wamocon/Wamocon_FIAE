...
export async function generateActivityReportPDF(
    ...
    const drawSectionHeading = (title: string, y: number): number => {
        ...
        doc.rect(margin, y - 3.5, 1.5, 5.5, 'F');
        // Heading text
        doc.setFontSize(11);
    ...
    const ensureSpace = (currentY: number, needed: number, resetY: number = 25): number => {
    ...
    doc.setFont('helvetica', 'bold');
    setColor(COLORS.primary);
    doc.text('T\u00c4TIGKEITSNACHWEIS', pageWidth / 2, yPos, { align: 'center' });
    yPos += 6;

    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    setColor(COLORS.secondary);
    doc.text('W\u00f6chentlicher Ausbildungsnachweis gem\u00e4\u00df \u00a7 13 BBiG', pageWidth / 2, yPos, { align: 'center' });
    yPos += 6;
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(`${formatDate(report.periodStart)}  –  ${formatDate(report.periodEnd)}`, col1ValX, metaY);
    ...
    const tableHead = [['Komp.', 'Tätigkeit / Use Case', 'Plan (Std)', 'IST (Std)', 'Anmerkungen']];
    ...
    const isTotalOverbooked = totalActual > totalPlanned;
    const MAX_WEEKLY_HOURS = 40;
    const exceeds40h = totalActual > MAX_WEEKLY_HOURS;

    const tableFoot = [['', `Gesamtstunden (max. ${MAX_WEEKLY_HOURS} Std./Woche):`, `${totalPlanned}`, `${totalActual}`, '']];
    ...
    autoTable(doc, {
        ...
        columnStyles: {
            0: { cellWidth: 18, halign: 'center' as const, fontStyle: 'bold' as const },
        ...
        didParseCell: (data) => {
            if (data.section === 'body' && data.column.index === 3 && overbookedRows.includes(data.row.index)) {
                data.cell.styles.textColor = COLORS.danger;
                data.cell.styles.fontStyle = 'bold';
            }
            if (data.section === 'foot' && data.column.index === 3 && exceeds40h) {
                data.cell.styles.textColor = [255, 100, 100];
                data.cell.styles.fontStyle = 'bold';
            } else if (data.section === 'foot' && data.column.index === 3 && isTotalOverbooked) {
                data.cell.styles.textColor = [255, 200, 200];
            }
        },
        willDrawCell: (data) => {
            if (data.section === 'body' && overbookedRows.includes(data.row.index)) {
                doc.setFillColor(COLORS.overbookedBg[0], COLORS.overbookedBg[1], COLORS.overbookedBg[2]);
    ...
    // Hours summary — factual only, no incorrect legal citations
    if (isTotalOverbooked) {
        doc.setFontSize(7.5);
        setColor(COLORS.danger);
        doc.setFont('helvetica', 'italic');
        const diff = totalActual - totalPlanned;
        doc.text(`Hinweis: ${diff} Std. \u00fcber dem geplanten Umfang`, margin, yPos);
        setColor(COLORS.primary);
        yPos += 4;
    }
    ...
    if (exceeds40h) {
        ...
        setColor(COLORS.danger);
        doc.setFont('helvetica', 'bold');
        doc.text(`Achtung: ${totalActual} Std. \u2013 vertragliche Wochenarbeitszeit von ${MAX_WEEKLY_HOURS} Std. \u00fcberschritten`, margin, yPos);
        setColor(COLORS.primary);
        yPos += 4;
    } else {
        // No line needed when hours are within normal range
    }
    ...
    if (effectiveComment) {
        ...
        doc.setFontSize(9);
        doc.setFont('helvetica', 'italic');
        const splitComment = doc.splitTextToSize(effectiveComment, contentWidth - 10);
    ...
    doc.setLineWidth(0.3);
    doc.line(traineeBoxX + 5, yPos + 20, traineeBoxX + sigBoxWidth - 5, yPos + 20);

    doc.setFontSize(7);
    if (traineeSigned) {
        setColor(COLORS.success);
        doc.setFont('helvetica', 'bold');
        doc.text(`Bestätigt am: ${formatDate(report.traineeSignedAt)}`, traineeBoxX + 5, yPos + 26);
    } else {
    ...
    doc.setDrawColor(COLORS.primary[0], COLORS.primary[1], COLORS.primary[2]);
    doc.setLineWidth(0.3);
    doc.line(trainerBoxX + 5, yPos + 20, trainerBoxX + sigBoxWidth - 5, yPos + 20);

    doc.setFontSize(7);
    if (trainerSigned) {
        setColor(COLORS.success);
        doc.setFont('helvetica', 'bold');
        doc.text(`Bestätigt am: ${formatDate(report.trainerSignedAt)}`, trainerBoxX + 5, yPos + 26);
    } else {
        setColor(COLORS.warning);
        doc.setFont('helvetica', 'italic');
    ...
    setColor(COLORS.secondary);
    doc.setFont('helvetica', 'normal');
    const verificationText = `Dieser Ausbildungsnachweis wurde elektronisch erstellt und best\u00e4tigt. `
        + `Er dient als w\u00f6chentlicher T\u00e4tigkeitsnachweis gem\u00e4\u00df \u00a7 13 BBiG und dokumentiert die durchgef\u00fchrten `
    ...
    const watermarkText = report.status === 'DRAFT' ? 'ENTWURF'
        : report.status === 'SUBMITTED' ? 'EINGEREICHT'
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        // Right: Page number
        doc.text(`Seite ${i} / ${totalPages}`, pageWidth - margin, footerY + 2, { align: 'right' });
    }
