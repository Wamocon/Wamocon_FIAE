    ...
    <div className="p-6 max-w-7xl mx-auto space-y-8">
      ...
      <div className="glass-effect rounded-3xl p-8 shadow-lg border border-accent/30">
        <div className="text-center mb-6">
          ...
          <h1 className="text-3xl font-bold text-foreground mb-2">Module Übersicht</h1>
          <p className="text-muted">Entdecke alle verfügbaren Lernmodule und wähle deinen nächsten Schritt</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {modules.map((module) => {
          ...
          return (
            <div key={module.moduleId} className="glass-effect rounded-3xl shadow-lg border border-accent/30 overflow-hidden">
              {/* Module Header */}
              <div className="p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-muted">{completedLessons} abgeschlossen</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Clock className="w-4 h-4 text-muted" />
                      <span className="text-muted">{totalLessons} Lektionen</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {isExpanded && module.chapters && (
                <div className="px-6 pb-6 border-t border-accent/20">
                  <h4 className="font-semibold text-foreground mb-4 mt-4">Kapitel in diesem Modul</h4>
                  <div className="space-y-3">
                    {module.chapters.map((chapter: any) => {
                      const chapterProgress = chapter.lessons?.filter((l: any) => l.completed).length || 0
                      const totalChapterLessons = chapter.lessons?.length || 0
                      const chapterPercentage = totalChapterLessons > 0 ? Math.round((chapterProgress / totalChapterLessons) * 100) : 0
                      ...
                      return (
                        <div 
                          ...
                          onClick={() => handleChapterClick(module.moduleId, chapter.chapterId)}
                        >
                          ...
                          <div className="w-full bg-muted/30 rounded-full h-2 mb-2">
                            <div 
                              className={`h-2 rounded-full transition-all duration-500 ${
                                chapterPercentage === 100 ? 'bg-green-500' : 'bg-accent'
                              }`}
                          ...
                          <p className="text-xs text-muted">
                            {chapterProgress} von {totalChapterLessons} Lektionen abgeschlossen
                          </p>
                        </div>
                      )
                    })}
      ...
      <div className="glass-effect rounded-3xl p-8 shadow-lg border border-accent/30">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="text-center p-6 bg-background/50 rounded-2xl border border-accent/20">
            ...
            <h3 className="text-lg font-semibold text-foreground mb-2">Abgeschlossene Module</h3>
          ...
          <div className="text-center p-6 bg-background/50 rounded-2xl border border-accent/20">
            ...
            <h3 className="text-lg font-semibold text-foreground mb-2">Aktive Module</h3>
          ...
          <div className="text-center p-6 bg-background/50 rounded-2xl border border-accent/20">
            ...
            <h3 className="text-lg font-semibold text-foreground mb-2">Verbleibende</h3>
