...
function Dialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="glass-effect border-accent/30 bg-background relative z-10 w-full max-w-5xl rounded-3xl border p-0 shadow-2xl">
        {children}
      </div>
    </div>
  );
...
function Switch({
  ...
  return (
    ...
    >
      <span
        className={`bg-background inline-block h-5 w-5 transform rounded-full shadow transition-transform ${checked ? 'translate-x-5' : 'translate-x-1'}`}
      />
    </button>
  );
}
    ...
    <div className="flex gap-4">
      {options.map(opt => (
        ...
        >
          <input
            type="radio"
            className="border-border h-4 w-4 text-blue-600 focus:ring-blue-500"
...
function MultiSelect({
  ...
  const resolvedEmptyLabel = emptyLabel || t('quiz.managementNoResults');
  const resolvedDoneLabel = doneLabel || t('quiz.managementDone');
  const containerRef = useState<HTMLDivElement | null>(null)[0] as any;
  const setContainerRef = (el: HTMLDivElement | null) => {
    (MultiSelect as any)._ref = el;
  };
  const ref: React.MutableRefObject<HTMLDivElement | null> = useMemo(
    () => ({ current: (MultiSelect as any)._ref || null }),
    []
  );
  ...
  return (
    <div className="relative" ref={assignRef}>
      <div className="mb-2 flex flex-wrap gap-2">
        {selected.map(id => {
          ...
          return (
            <span
              key={id}
              className="bg-accent/20 text-foreground inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs"
            >
              {label}
              <button
                onClick={() => toggle(id)}
      ...
      {open && (
        <div className="border-accent/30 bg-background absolute z-30 mt-2 max-h-60 w-full overflow-auto rounded-md border p-1 shadow-lg">
          {filtered.length === 0 && (
          ...
          {filtered.map(opt => {
            ...
            return (
              <button
                ...
                className={`hover:bg-accent/20 flex w-full items-center justify-between rounded px-2 py-2 text-left text-sm ${active ? 'text-foreground' : 'text-foreground'}`}
              >
                <span>{opt.label}</span>
          ...
          <div className="border-accent/30 bg-background/80 sticky bottom-0 flex justify-end border-t p-1">
            <Button
              variant="secondary"
...
export function QuizManagement() {
  ...
  const openEdit = async (id: string) => {
    try {
      ...
      setAssigned(
        Array.isArray(data.assigned_trainee_ids)
  ...
  const setCorrect = (id: string, idx: 0 | 1 | 2 | 3) =>
    setQuestions(qs =>
      qs.map(q => (q.id === id ? { ...q, correctIndex: idx } : q))
    );
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (editMode && editId) {
        ...
        if (!res.ok) throw new Error(t('quiz.managementSaveError'));
        ...
        if (!res.ok) throw new Error(t('quiz.managementCreateError'));
  ...
  const loadTrainees = async () => {
    ...
    try {
      ...
      if (!res.ok) return;
      const data = await res.json();
      const opts: ComboOption[] = (data.trainees || []).map((t: any) => ({
        id: t.id,
        label: t.full_name,
      }));
  ...
  async function syncQuizMembers(
    quizId: string,
    desired: string[],
    addedById: string
  ) {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 p-6">
      ...
      <div className="glass-effect border-accent/30 rounded-3xl border p-6 shadow-lg">
        <div className="overflow-x-auto">
          <table className="divide-border min-w-full divide-y">
            <thead className="divide-border divide-y">
              <tr>
                <th className="text-foreground px-4 py-3 text-sm font-medium">
                  {t('quiz.managementTableTitle')}
                </th>
                <th className="text-foreground px-4 py-3 text-sm font-medium">
                  {t('quiz.managementTableStatus')}
                </th>
                <th className="text-foreground px-4 py-3 text-sm font-medium">
                  {t('quiz.managementTableAssigned')}
                </th>
                <th className="text-foreground px-4 py-3 text-sm font-medium">
                  {t('quiz.managementTableCreated')}
                </th>
                <th className="text-foreground px-4 py-3 text-sm font-medium">
                  {t('quiz.managementTableActions')}
                </th>
              </tr>
            </thead>
              ...
              {quizzes.map(q => (
                <tr key={q.id} className="hover:bg-accent/10 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    {q.is_active ? (
                      <span className="inline-flex items-center rounded-full bg-green-500/20 px-2.5 py-1 text-xs font-medium text-green-600 dark:text-green-400">
                        {t('quiz.managementActive')}
                      </span>
                    ) : (
                      <span className="bg-muted text-muted-foreground inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium">
                        {t('quiz.managementInactive')}
                      </span>
                    )}
                  </td>
                  <td className="text-foreground px-4 py-3 text-sm">
                    {q.assigned_count} {t('quiz.managementTrainees')}
                  </td>
                  <td className="text-foreground px-4 py-3 text-sm">
      ...
      <Dialog open={open} onClose={() => setOpen(false)}>
        <div className="border-accent/30 bg-background flex items-start justify-between rounded-t-3xl border-b px-6 py-4">
          <div>
            <h3 className="text-foreground text-lg font-semibold">
              {editMode
                ? t('quiz.managementDialogEditTitle')
                : t('quiz.managementDialogCreateTitle')}
            </h3>
            <p className="text-muted text-sm">
              {editMode
                ? t('quiz.managementDialogHelpEdit')
                : t('quiz.managementDialogHelpCreate')}
            </p>
          </div>
          <button
            onClick={() => setOpen(false)}
        ...
        <div className="flex items-center gap-3 px-6 pt-4">
          <div
            className={`h-2 flex-1 rounded-full ${step >= 0 ? 'bg-blue-600' : 'bg-muted'}`}
          />
          <div
            className={`h-2 flex-1 rounded-full ${step >= 1 ? 'bg-blue-600' : 'bg-muted'}`}
          />
          <div
            className={`h-2 flex-1 rounded-full ${step >= 2 ? 'bg-blue-600' : 'bg-muted'}`}
          />
        </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  placeholder={t('quiz.managementTitlePlaceholder')}
                />
              </div>
              <div className="border-accent/30 bg-muted/30 flex items-center justify-between rounded-lg border p-3">
                <div>
                  <div className="text-foreground text-sm font-medium">
                    {t('quiz.managementSetActive')}
                  </div>
                  <div className="text-muted text-xs">
                    {t('quiz.managementSetActiveDesc')}
                  </div>
                </div>
            ...
            <div className="space-y-4">
              {questions.map((q, qi) => (
                <div key={q.id} className="border-border rounded-lg border p-4">
                  <div className="mb-3 flex items-center justify-between">
                    <div className="text-foreground text-sm font-medium">
                      {t('quiz.managementQuestion')} {qi + 1}
                    </div>
                    <button
                  ...
                  <div className="mb-3">
                    <label className="text-foreground mb-1 block text-xs font-medium">
                      {t('quiz.managementQuestionText')}
                    </label>
                    <Input
                      value={q.text}
                      onChange={e => updateQuestionText(q.id, e.target.value)}
                      placeholder={t('quiz.managementQuestionPlaceholder')}
                    />
                  </div>
                    ...
                    {q.options.map((opt, idx) => (
                      <div key={idx}>
                        ...
                        <Input
                          ...
                          placeholder={`${t('quiz.managementOption')} ${idx + 1}`}
                        />
                      </div>
                    ))}
                  </div>
                  <div className="mt-3">
                    <label className="text-foreground mb-1 block text-xs font-medium">
                      {t('quiz.managementCorrectAnswer')}
                    </label>
                    <RadioGroup
                      value={String(q.correctIndex)}
                      onChange={v =>
                        setCorrect(q.id, Number(v) as 0 | 1 | 2 | 3)
                      }
                      options={[
                        {
                          value: '0',
                          label: `${t('quiz.managementOption')} 1`,
                        },
                        {
                          value: '1',
                          label: `${t('quiz.managementOption')} 2`,
                        },
                        {
                          value: '2',
                          label: `${t('quiz.managementOption')} 3`,
                        },
                        {
                          value: '3',
                          label: `${t('quiz.managementOption')} 4`,
                        },
                      ]}
                    />
                  </div>
              ...
              <div>
                <Button
                  ...
                  onClick={addQuestion}
                >
                  <Plus className="h-4 w-4" /> {t('quiz.managementAddQuestion')}
                </Button>
              </div>
            </div>
          </div>
        ...
        {step === 2 && (
          <div className="h-[20vh] overflow-y-auto px-6 py-4">
            <div className="space-y-6">
              <div>
                <label className="text-foreground mb-1 block text-sm font-medium">
                  {t('quiz.managementAssignTrainees')}
                </label>
                <MultiSelect
                  options={traineeOptions}
                  selected={assigned}
                  onChange={setAssigned}
                  placeholder={t('quiz.managementSearchTrainees')}
                  emptyLabel={t('quiz.managementNoResults')}
                  doneLabel={t('quiz.managementDone')}
                />
              </div>
              <div>
                <label className="text-foreground mb-1 block text-sm font-medium">
                  {t('quiz.managementCollaborators')}
                </label>
                <MultiSelect
                  options={trainerOptions}
                  selected={collaborators}
                  onChange={setCollaborators}
                  placeholder={t('quiz.managementSearchTrainers')}
                  emptyLabel={t('quiz.managementNoResults')}
                  doneLabel={t('quiz.managementDone')}
                />
              </div>
        ...
        {/* Footer */}
        <div className="border-accent/30 bg-background sticky bottom-0 flex items-center justify-between gap-3 border-t px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            >
              {t('quiz.managementBack')}
            </Button>
            <Button
              onClick={() => setStep(s => (s < 2 ? ((s + 1) as 0 | 1 | 2) : s))}
              disabled={step === 2}
            >
              {t('quiz.managementNext')}
            </Button>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="ghost" onClick={() => setOpen(false)}>
              {t('quiz.managementCancel')}
            </Button>
            <Button onClick={onSave} disabled={!canSave || saving}>
              {editMode
                ? t('quiz.managementSaveChanges')
                : t('quiz.managementSaveQuiz')}
            </Button>
          </div>
        </div>
      </Dialog>
