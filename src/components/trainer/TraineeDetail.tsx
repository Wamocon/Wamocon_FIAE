      ...
      <div className="glass-effect rounded-3xl p-8 shadow-lg border border-accent/30">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          <div className="flex items-center gap-6">
            ...
            <div>
              <h1 className="text-3xl font-bold text-foreground mb-2">{mockTrainee.name}</h1>
        ...
        <div className="glass-effect rounded-3xl p-6 shadow-lg border border-accent/30">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-foreground">{mockTrainee.progress}%</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-8">
          {activeTab === 'overview' && (
            <div className="space-y-6">
              <h3 className="text-xl font-bold text-slate-800 mb-4">Aktuelle Lernaktivitäten</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="p-6 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl border border-blue-200">
                  <h4 className="font-semibold text-blue-800 mb-2">Aktuelles Modul</h4>
                  <p className="text-blue-700">Variablen und Datentypen</p>
                  <div className="mt-3 w-full bg-blue-200 rounded-full h-2">
                    <div className="bg-blue-600 h-2 rounded-full" style={{ width: '75%' }}></div>
                  </div>
                  <p className="text-sm text-blue-600 mt-1">75% abgeschlossen</p>
                </div>
                
                <div className="p-6 bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl border border-green-200">
                  <h4 className="font-semibold text-green-800 mb-2">Nächster Termin</h4>
          ...
          {activeTab === 'progress' && (
            <div className="space-y-6">
              <h3 className="text-xl font-bold text-slate-800 mb-4">Lernfortschritt</h3>
              <div className="space-y-4">
                {mockData.curriculum.map((module) => (
                  <div key={module.moduleId} className="p-4 bg-slate-50 rounded-2xl border border-slate-200">
          ...
          {activeTab === 'submissions' && (
            <div className="space-y-6">
              <h3 className="text-xl font-bold text-slate-800 mb-4">Eingereichte Tests</h3>
              <div className="bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-slate-100">
                      <tr>
                        <th className="px-6 py-4 text-left text-sm font-medium text-slate-700">Quiz Titel</th>
                        <th className="px-6 py-4 text-left text-sm font-medium text-slate-700">Ergebnis</th>
                        <th className="px-6 py-4 text-left text-sm font-medium text-slate-700">Status</th>
                        <th className="px-6 py-4 text-left text-sm font-medium text-slate-700">Datum</th>
                        <th className="px-6 py-4 text-left text-sm font-medium text-slate-700">Aktion</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-200">
                      {mockSubmissions.map((submission) => (
                        <tr key={submission.submissionId} className="hover:bg-white transition-colors duration-200">
                          <td className="px-6 py-4">
                            <div className="font-medium text-slate-800">{submission.quizTitle}</div>
                          </td>
                          <td className="px-6 py-4">
                          ...
                          <td className="px-6 py-4">
                            <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                              ...
                              {submission.status === 'reviewed' ? 'Bewertet' : 'Ausstehend'}
                            </span>
                          </td>
          ...
          {activeTab === 'notes' && (
            <div className="space-y-6">
              <h3 className="text-xl font-bold text-slate-800 mb-4">Notizen & Feedback</h3>
              <div className="space-y-4">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm text-yellow-800">
                        <strong>Verbesserungsvorschlag:</strong> Elias könnte mehr praktische Übungen zu Variablen machen.
                      </p>
                      <p className="text-xs text-yellow-600 mt-1">Notiert am 15.01.2025</p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm text-green-800">
                        <strong>Positives Feedback:</strong> Sehr gute Leistung im Grundbegriffe-Quiz!
                      </p>
                      <p className="text-xs text-green-600 mt-1">Notiert am 12.01.2025</p>
                    </div>
                  </div>
                </div>
              </div>
