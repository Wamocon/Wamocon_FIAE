...
interface SoftskillCriterion {
  ...
  description: string;
  kLevel: string;
  competencyArea: 'FACHKOMPETENZ' | 'METHODENKOMPETENZ' | 'PERSONALKOMPETENZ';
  orderIndex: number;
}
...
export default function SoftskillRatingGrid({
  ...
  const [groupedCriteria, setGroupedCriteria] = useState<
    Record<string, SoftskillCriterion[]>
  >({});
    ...
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
      {Object.entries(groupedCriteria).map(([area, areaCriteria]) =>
        areaCriteria.map(criterion => {
          ...
          const colorClass =
            COMPETENCY_LABELS[area]?.color || 'bg-gray-500/20 text-gray-400';

          return (
            <div
              key={criterion.id}
              className={`border-border/50 bg-card hover:border-border rounded-xl border p-4 transition-all ${showTrainerRatings ? getDeviationColor(selfRating, trainerRating) : ''}`}
            >
              {/* Header */}
                ...
                <div className="flex items-center justify-between">
                  ...
                  {isTrainer ? (
                    <div className="flex items-center gap-2">
                      <span
                        className={`text-lg font-bold ${selfRating ? GRADE_OPTIONS.find(g => g.value === selfRating)?.color.split(' ')[1] : 'text-muted-foreground/30'}`}
                      >
                        {selfRating || '-'}
                      </span>
                    </div>
                    ...
                    <Select
                      ...
                      disabled={readOnly}
                    >
                      <SelectTrigger className="bg-muted/20 border-border h-9 w-[120px]">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {isTrainer && onTrainerChange ? (
                      <Select
                        value={trainerRating}
                        onValueChange={val =>
                          onTrainerChange(criterion.id, val)
                        }
                        disabled={readOnly}
                      >
                        <SelectTrigger className="h-9 w-[120px] border-emerald-500/20 bg-emerald-500/10 text-emerald-400">
                          <SelectValue placeholder="Note..." />
                        </SelectTrigger>
                        <SelectContent>
                          {GRADE_OPTIONS.map(grade => (
                      ...
                      <span
                        className={`text-lg font-bold ${trainerRating ? GRADE_OPTIONS.find(g => g.value === trainerRating)?.color.split(' ')[1] : 'text-muted-foreground/30'}`}
                      >
                        {trainerRating || '-'}
                      </span>
                    )}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {isTrainer && onReleaseChange ? (
                      <Select
                        value={releaseRating}
                        onValueChange={val =>
                          onReleaseChange(criterion.id, val)
                        }
                      >
                        <SelectTrigger className="h-9 w-[120px] border-amber-500/20 bg-amber-500/10 text-amber-400">
                          <SelectValue placeholder="Note..." />
                        </SelectTrigger>
                        <SelectContent>
                          {GRADE_OPTIONS.map(grade => (
                      ...
                      <span
                        className={`text-lg font-bold ${releaseRating ? GRADE_OPTIONS.find(g => g.value === releaseRating)?.color.split(' ')[1] : 'text-muted-foreground/30'}`}
                      >
                        {releaseRating || '-'}
                      </span>
                    )}
