...
function separateTasksAndSolutions(content: string): { tasksText: string; solutionsText: string } {
  ...
  pairs.forEach((pair, index) => {
    ...
    tasksLines.push(`${num}. ${pair.problemTitle}`);
    if (pair.problem) {
      tasksLines.push(`Szenario: ${pair.problem}`);
    }
    tasksLines.push('');

    // Solutions: Problem number + solution
    solutionsLines.push(`${num}. ${pair.problemTitle}`);
 ...
 */
export function parseProblemSolutionPairs(content: string): Array<ProblemSolutionPair> {
  const pairs: Array<ProblemSolutionPair> = [];
