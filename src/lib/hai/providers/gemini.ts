...
export class GeminiChatProvider implements ChatProvider {
    ...
    constructor(apiKey: string | undefined, modelName: string = 'gemini-2.5-flash') {
        if (apiKey) {
    ...
    async generateResponse(
        ...
        if (!this.model) {
            throw new Error('HAI.ai [Gemini Chat]: Not initialized. Check GEMINI_API_KEY.');
        }
        ...
        const history = [
            ...
            { role: 'model' as const, parts: [{ text: 'Verstanden. Ich bin HAI.ai, dein digitaler Lernbegleiter.' }] },
            ...messages.map(m => ({
    ...
    async generateResponseStream(
        ...
        const result = await this.callWithRetry(() => chat.sendMessageStream(userMessage));
        let fullText = '';
    ...
    private async callWithRetry<T>(fn: () => Promise<T>, retries = 3, delay = 1000): Promise<T> {
        try {
            ...
            if (retries > 0 && isRetryable) {
                const waitTime = (status === 429 || message.includes('429')) ? Math.max(delay, 3000) : delay;
                console.warn(`HAI.ai [Gemini Chat]: ${status || 'Error'}. Retrying in ${waitTime}ms... (${retries} left)`);
...
export class GeminiEmbeddingProvider implements EmbeddingProvider {
    ...
    async generateEmbedding(text: string): Promise<EmbeddingResult> {
        if (!this.model) {
            throw new Error('HAI.ai [Gemini Embedding]: Not initialized. Check GEMINI_API_KEY.');
        }
        ...
        const result = await this.callWithRetry(async () => {
            return this.model!.embedContent({
                ...
                // This keeps 768 dims so we don't need to alter the DB column
    ...
    private async callWithRetry<T>(fn: () => Promise<T>, retries = 3, delay = 2000): Promise<T> {
        try {
            ...
            if (retries > 0 && isRetryable) {
                ...
                const waitTime = status === 429 || message.includes('429') ? Math.max(delay, 5000) : delay;
                console.warn(
