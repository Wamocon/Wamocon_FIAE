...
import db from '@/db';
import { haiEmbeddings, contentDocuments, useCases } from '@/db/migrations/schemas/schema';
...
export async function ingestUseCaseDocument(
    ...
    try {
        ...
        const extraction: PerPageExtractionResult = await extractTextByPage(docRow.storageUrl);
        ...
        onProgress?.(10, 100, `Extracted ${extraction.totalPages} pages, generating embeddings...`);
        ...
        for (let pageIdx = 0; pageIdx < extraction.pages.length; pageIdx++) {
            ...
            for (let chunkIdx = 0; chunkIdx < chunks.length; chunkIdx++) {
                ...
                if (!forceReindex) {
                    const exists = await pageChunkExists(useCaseId, page.page, chunkIdx, contentHash);
                    if (exists) {
                ...
                try {
                    ...
                    // Generate embedding
                    const embeddingResult = await embeddingProvider.generateEmbedding(chunkText);
                    ...
                    console.error(`Failed to embed chunk ${chunkIdx} of page ${page.page}:`, chunkError);
                    result.chunksFailed++;
                }
            }
            ...
            onProgress?.(progressPercent, 100, `Processed page ${page.page}/${totalPages}`);
        }
...
export async function ingestAllUseCases(
    ...
    try {
        ...
        onProgress?.(0, docs.length, `Found ${docs.length} use case documents to process`);
 ...
 */
export async function deleteUseCaseEmbeddings(useCaseId: string): Promise<{ deleted: number }> {
    try {
        const result = await db.execute(sql`
