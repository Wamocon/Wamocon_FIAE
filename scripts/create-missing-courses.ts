...
async function createMissingCourses() {
  ...
  if (existing25.length === 0) {
    const [c25] = await db.insert(courses).values({
      title: 'Kapitel 25 - Zusätzliche Kompetenzen',
      description: 'Use Cases für zusätzliche FIAE Kompetenzen',
      year: 3,
      chapter: 25,
  ...
  if (existing26.length === 0) {
    const [c26] = await db.insert(courses).values({
      title: 'Kapitel 26 - Erweiterte Kompetenzen',
      description: 'Use Cases für erweiterte FIAE Kompetenzen',
      year: 3,
      chapter: 26,
      createdById: trainer.id,
      isActive: true,
